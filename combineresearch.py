...
def summarize_video(video_path, frame_rate, k_motion, k_color, threshold, max_frames, output_path):
    ...
    # Use event frames as input for the next technique
    motion_features_event = extract_motion(event_frames)
    key_frames_motion_event = apply_kmeans_motion(event_frames, motion_features_event, k_motion)

    # Use key frames based on motion (from event technique) as input for the color technique
    histograms = extract_histograms(key_frames_motion_event)
    key_frame_indices_color = apply_kmeans_color(histograms, k_color)
    key_frames_color = key_frames_motion_event[key_frame_indices_color]

    create_event_summary(key_frames_color, output_path)
...
if __name__ == '__main__':
    ...
    output_path = 'C:/Users/admin/OneDrive/Desktop/VideoSum/static_combinedresearch_summary'
