...
def save_frames_with_objects(frames, output_path):
    count = 0
    for frame in frames:
        class_ids, _, _ = detect_objects(frame)
        if len(class_ids) > 0:
            image_path = os.path.join(output_path, f"frame_{count}.jpg")
...
def summarize_video(video_path, frame_rate, output_path):
    ...
    save_frames_with_objects(frames, output_path)
    print(f"Video summarization completed. {len(frames)} frames processed.")

# Example usage
video_path ='v21.mpg'
frame_rate = 20  # Extract one frame every 10 seconds
output_path = "C:/Users/admin/OneDrive/Desktop/VideoSum/staticobject.summary"
