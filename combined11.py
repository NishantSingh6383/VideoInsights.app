...
def apply_kmeans_motion_color(frames, motion_features, histograms, k):
    min_length = min(len(motion_features), len(histograms))
    motion_features = np.array(motion_features[:min_length]).reshape(-1, 1)
    histograms = np.array(histograms[:min_length])
    features = np.concatenate((motion_features, histograms), axis=1)
...
def summarize_video(video_path, frame_rate, k_motion, k_color, threshold, max_frames, output_path):
    ...
    key_frames_motion_color = apply_kmeans_motion_color(frames, motion_features, histograms, k_motion)
    ...
    print(f"Video summarization completed. Key frames based on motion and color: {len(key_frames_motion_color)}. Event frames: {len(event_frames)}.")
...
if __name__ == '__main__':
    ...
    output_path = 'C:/Users/admin/OneDrive/Desktop/VideoSum/static_combined11_summary'
